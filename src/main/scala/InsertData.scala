...
class InsertData ( developers: Developers , genres: Genres, publishers: Publishers, games: Games, users: Users, comments: Comments ) {
	...
	def FillUser(){
		users.createUser(1, "Jean-Edouard", "de la FesMol", "JEF@gmail.com", "root", "addressJEF", "0666666666", "root");
		users.createUser(2, "Marie", "Kartiol", "MK@gmail.com", "root", "addressMK", "0777777777", "root");
		users.createUser(3, "Nabila", "Aliban", "NA@gmail.com", "root", "addressNA", "0888888888", "root");
		users.createUser(4, "Paul", "Sambousek", "PS@gmail.com", "root", "addressPS", "0111111111", "root");
		users.createUser(5, "Carlita", "Walla", "CW@gmail.com", "root", "addressCW", "0222222222", "root");
	}

	def FillComment(){
		comments.createComment(1, 1, 0, "Le jeu est bien mais la.cartouche bug souvent.", 3);
		comments.createComment(2, 1, 0, "Juste une tuerie ce jeu", 5);
		comments.createComment(3, 1, 55, "Très bon jeu les graphismes sont très bien.", 5);
		comments.createComment(4, 1, 189, "Super jeux avec une remise et reçu quelques jours après par mail un code pour le jeu pour débloquer super cool", 5);
		comments.createComment(5, 2, 0, "En 4 heures de jeux , j’ai eux 9 crash ou le jeu se ferme seul sans sauvegarder , j’ai eux aussi un chargement en plein liberté qui a duré 10 min puis j’ai éteint sa avancer pas plus .", 1);
		comments.createComment(6, 2, 2, "Le jeu crash trop souvent", 2);
		comments.createComment(7, 2, 4, "Reçu en très bon état mais je n'ai pas reçu le bonus de précommande (mission bonus) qui était promis.", 4);
		comments.createComment(8, 3, 0, "Je mets 4 étoiles parce que le jeu est bien et qu'il y a plusieurs niveaux", 4);
		comments.createComment(9, 3, 2, "0 bug très fluide monde ouvert très beau jeu mes mdr facile de faire un monde ouvert sans scénario y a rien il manque cruellement d échange et de personnages c est monotones et ennuyeux pour un jeu avec temps de potentiel.", 3);
		comments.createComment(10, 4, 0, "Super jeu!", 5);
		comments.createComment(11, 1, 189, "Une bouse sans nom", 1);
		comments.createComment(12, 2, 174, "Livraison rapide , jeux vraiment fun", 4);
		comments.createComment(13, 3, 174, " le jeu est vraiment mauvais", 1);
		comments.createComment(14, 1, 62, "Rien à redire sur le rpoduit. En revanche la boîte du jeu à été abîmée lors de la livraison.", 1);
		comments.createComment(15, 2, 62, "très bon jeu, par contre je n ai pas reçu le code de mission bonus alors que je l’ai précommandé .", 4);
		comments.createComment(16, 3, 103, "Jeux pas trop mal, dommage qu'il n'est pas de mode versus", 3);
		comments.createComment(17, 1, 74, "J’ai joué 2 jours car ce jeu est nul", 2);
		comments.createComment(18, 2, 74, "Très bon jeu arrivée en avance bien emballé bon état fonctionne très bien", 5);
		comments.createComment(19, 3, 74, "Livré sous blister envoie du code cadeau par mail", 5);
		comments.createComment(20, 2, 136, "J’adore ! Enfin un jeu a monde ouvert.", 5);
